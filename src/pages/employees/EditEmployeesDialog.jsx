...
const EditEmployeesDialog = (props) => {
  const { editEmployeesDialogOpen, setEditEmployeesDialogOpen, getEmployeeList, selectedEmployee, setSelectedEmployee } = props

  console.log(selectedEmployee)

  const joinedSubsidiarie = useUserSessionStore((state) => state.joinedSubsidiarie)

  const { control, handleSubmit, reset } = useForm({ defaultValues: createEmployeesDefaultValues(selectedEmployee) })

  const { trueFalseOptions, functionsOptions } = useSelectOptionsStore()

  useEffect(() => {
    if (selectedEmployee) {
      reset(createEmployeesDefaultValues(selectedEmployee))
    }
  }, [selectedEmployee, reset])

  const onClose = () => {
    reset(createEmployeesDefaultValues())
